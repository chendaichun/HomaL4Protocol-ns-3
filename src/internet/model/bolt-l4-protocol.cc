...
TypeId BoltL4Protocol::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::BoltL4Protocol")
          ...
          .AddAttribute(
              "KValue",              
              "A double value representing k, initialized to a large integer with many factors.", 
              DoubleValue(193993800.0), 
              MakeDoubleAccessor(&BoltL4Protocol::m_k), 
...
std::string BoltL4Protocol::CcModeToString(enum CcMode_e ccMode) {
  switch (ccMode) {
    case CC_DEFAULT:
      return "DEFAULT";
    case CC_SWIFT:
...
void BoltOutboundMsg::HandleAck(BoltHeader const &boltHeader) {
  ...
  NS_ASSERT(m_rtt > 0);
  m_basertt = std::min((double)m_rtt, m_basertt);
  NS_ASSERT(m_basertt > 0);
  m_basertt = m_basertt/1000*1000;
  ...
  while (!m_msgFinSeqList.empty() && m_ackNo >= m_msgFinSeqList.front()) {
...
void BoltOutboundMsg::HandleNack(BoltHeader const &boltHeader) {
  ...
  // double base_rtt = m_bolt->m_k;
  // base_rtt = std::min((double)m_rtt, base_rtt);
  // base_rtt = (uint64_t)base_rtt;
  // NS_ASSERT(base_rtt > 0);
...
void BoltOutboundMsg::SendDownOnePacket(uint32_t payloadSize) {
  ...
  if (flag & BoltHeader::Flags_t::LAST) {
    // std::ofstream logFile("last_packets_sent.log", std::ios::app);
    // logFile << Simulator::Now().GetNanoSeconds()
    //        << " TxMsgId:" << m_txMsgId
    //        << " SeqNo:" << m_sndNext
    //        << " PayloadSize:" << payloadSize
    //        << " FlowSize:" << m_flowSizeBytes
    //        << " Cwnd:" << m_cwnd
    //        << " Progress:" << m_sndNext << "/" << m_flowSizeBytes
    //        << " LAST_COUNT:" << lastPacketCount
    //        << " " << (flag & BoltHeader::Flags_t::FIN ? "FIN" : "")
    //        << std::endl;
    // logFile.close();
  }
  ...
  //bolth.SetSrttNs(static_cast<uint32_t>(m_rtt));
  double weight = 0;
  if (m_basertt > 0)
  {
     weight = m_bolt->m_k / static_cast<double>(m_basertt) / 1000;
     if(weight < 1) weight = 1;
  }
  bolth.SetSrttNs(static_cast<uint8_t>(weight));
  p->AddHeader(bolth);
...
void BoltSendScheduler::CtrlPktRecvdForOutboundMsg(
  ...
  // lhy debug 0808
  if((targetMsg->GetSrcAddress() == ipv4Header.GetDestination()) &&
            (targetMsg->GetDstAddress() == ipv4Header.GetSource()) &&
            (targetMsg->GetSrcPort() == boltHeader.GetDstPort()) &&
            (targetMsg->GetDstPort() == boltHeader.GetSrcPort()))
            {
