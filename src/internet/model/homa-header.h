...
public:
  ...
  /**
   * \brief Set transport feedback flags (e.g., SIRD csn)
   * \param flags feedback flags bitfield
   */
  void SetFeedbackFlags (uint8_t flags);
  /**
   * \brief Get transport feedback flags
   * \return feedback flags bitfield
   */
  uint8_t GetFeedbackFlags () const;
...
//   uint16_t GetIncoming (void) const;
  ...
  /**
   * \brief Feedback bits for SIRD-compatible signaling
   */
  typedef enum FeedbackFlags_t
  {
    FEEDBACK_NONE = 0, //!< No feedback
    FEEDBACK_CSN = 1   //!< Congested Sender Notification
  } FeedbackFlags_t;
...
private:
  ...
  uint8_t m_feedbackFlags;//!< Transport feedback bits (e.g., csn)
