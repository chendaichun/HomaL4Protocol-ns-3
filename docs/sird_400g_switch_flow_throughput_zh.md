...
为了判断该场景是否能够证明“无效授权导致吞吐丢失”，需要进一步看交换机到每个 receiver 的最后一跳链路是否被 DATA 包打满。这里将交换机侧逐流 trace 按 `dstHost` 聚合，也就是把发往同一个 receiver 的 3 条 flow 在同一个 `100us` 窗口内相加，得到该 receiver 最后一跳的窗口吞吐。

结果如下：

| 协议 | 最后一跳 | 活跃窗口 | 活跃期平均吞吐 | 链路利用率 | 峰值吞吐 |
|---|---:|---:|---:|---:|---:|
| SIRD | switch -> host1 | 61 | 393.442Gbps | 98.36% | 397.472Gbps |
| SIRD | switch -> host2 | 61 | 393.442Gbps | 98.36% | 397.472Gbps |
| SIRD | switch -> host3 | 61 | 393.442Gbps | 98.36% | 397.472Gbps |
| SIRD | switch -> host4 | 61 | 393.442Gbps | 98.36% | 397.472Gbps |
| Homa | switch -> host1 | 62 | 387.097Gbps | 96.77% | 397.519Gbps |
| Homa | switch -> host2 | 62 | 387.097Gbps | 96.77% | 397.473Gbps |
| Homa | switch -> host3 | 61 | 393.443Gbps | 98.36% | 397.473Gbps |
| Homa | switch -> host4 | 61 | 393.443Gbps | 98.36% | 397.473Gbps |

这个结果说明：在刚刚这个 400G 单交换机 all-to-all 场景中，最后一跳链路在活跃传输阶段基本可以用满。SIRD 的四条最后一跳平均利用率均为 `98.36%`；Homa 的最后一跳平均利用率也在 `96.77%-98.36%` 之间。因此，该实验不能作为“无效授权导致最后一跳吞吐丢失”的核心证据。

更准确地说，这个实验说明当前对称 all-to-all workload 中 receiver 侧没有明显因为 credit 分配低效而长期空闲。若要证明无效授权问题，应改用更针对性的场景：receiver 仍有大量 scheduled 数据等待返回，但其最后一跳 DATA 吞吐明显低于链路带宽；同时可以看到 credit outstanding 被长 RTT sender 或 uplink 受限 sender 占住。
...
SIRD 的结果表现为 12 条 flow 几乎同步完成、活跃期吞吐接近一致。按目的 host 聚合后，四条最后一跳链路的活跃期平均利用率均达到 `98.36%`，说明在当前对称 all-to-all 场景中，SIRD 的 credit 控制没有造成 receiver downlink 长期空闲，也没有出现明显的吞吐丢失。Homa baseline 也能完成所有流，但逐流吞吐更有波动，部分流出现接近单链路上限的短时峰值，最后一跳平均利用率在 `96.77%-98.36%` 之间。
...
第三段写结果：实验显示 SIRD 与 Homa 均完成 12/12 条消息。SIRD 下 12 条 flow 的交换机侧吞吐曲线高度重合，活跃期平均吞吐约 `131Gbps`，完成时间约 `6.14ms`；按 receiver 聚合后，SIRD 四条最后一跳链路的活跃期平均吞吐均为约 `393.44Gbps`，即 `98.36%` 的 400G 链路利用率。Homa 下所有 flow 也完成，但逐流曲线更波动，部分 flow 出现接近 `395Gbps` 的短时峰值，最后一跳平均利用率在 `96.77%-98.36%` 之间。
